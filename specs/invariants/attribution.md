...
3. **Email match (fallback, `link_method = 'email_match'`)**: for orders predating the pixel or where the distinct_id was lost. Match `utm_attribution` email ↔ order email; if multiple, use the most recent within the 30-day window. Lower confidence — surfaced separately in reporting. **As of 2026-06-29**, when the matched touch itself carries a `posthog_distinct_id`, the order is stamped with it and a server-side `purchase_completed` is emitted (mirroring the pixel path) so email-matched conversions are visible in PostHog too. Previously this path linked the order in the DB but emitted nothing — those conversions counted toward `link_method` coverage yet stayed invisible to PostHog session/person joins, which is why the PostHog `purchase_completed` count sat below DB link coverage. A matched touch with no distinct_id still links the order; it just can't be attributed to a person.
