...
| GET | `/api/production/po/[id]/emails` | Emails mentioning this PO — searches the team's connected Gmail inboxes (subject OR body) for the PO number + its SKUs, deduped across inboxes, with a Gmail deep-link each. Admin-only. Loaded lazily by the **Related emails** section in the PO Activity tab (`po-related-emails`) so the page never blocks on Gmail |
