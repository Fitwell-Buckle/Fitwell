...
| GET | `/api/production/po/[id]/emails` | Emails mentioning this PO — searches the team's connected Gmail inboxes (subject OR body) for the PO number + its SKUs, with a Gmail deep-link each. Deduped across inboxes by **RFC822 Message-ID** (so a CC'd email — a different per-mailbox id in each inbox — collapses to one), preferring the **logged-in user's** copy. Admin-only. Loaded lazily by the PO Activity tab's `PoTimeline` (`showRelatedEmails`) and **merged chronologically into the notes/documents feed** (violet *Email* badge), so the page never blocks on Gmail |
