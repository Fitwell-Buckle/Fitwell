...
| GET | `/api/creators/signup-count` | Admin-only. `{ count }` of unreviewed self-registered creators (`source=self_registration` AND `vetting_status=unreviewed`) — drives the blue dot on the dedicated **Creator Signups** nav item (Marketing group; links to `/creators?source=self_registration`). Polled by the sidebar every 60s; clears when the team approves/rejects each signup |
