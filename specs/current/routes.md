...
| POST | `/api/compose/send` | Send a composed reply from the signed-in admin's Gmail — JSON `{ to, subject, body, cc?, bcc? }` (`cc`/`bcc` comma-separated email lists, validated). Same `gmail.send`/API-enabled 409s as the messages send route |
