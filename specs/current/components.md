...
| `crm/inbound-messages` | Client — `<InboundMessages emails={[…]} relationship />`. Full inbound email history for a customer/company/**supplier** across all connected team inboxes (`GET /api/inbound?emails=…`), color-coded per inbox with a filter, Gmail deep-links, per-message + "New message" Compose. Rendered on the consumer customer detail (`/customers/[id]`), B2B company detail (`/customers/brands/[id]`), and **supplier detail** (`/modules/production/suppliers/[id]`) pages |
| `crm/customer-messages-panel` | Client — surfaced at the top of the Customers B2B/Consumer tabs **and the Suppliers list** (`audience` = `b2b`/`consumer`/`supplier`). Lists undismissed `customer_message` rows (sender, subject, snippet, which inbox) with **Compose Message**, **Open in Gmail**, and **Dismiss** (`POST /api/customer-messages/[id]/dismiss`, optimistic). Server pages fetch via `listCustomerMessages(audience)` |
