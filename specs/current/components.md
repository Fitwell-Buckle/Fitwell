...
| `leads/[id]/lead-detail` | Client editor — stage/status/persona badges, "Convert to Company" picker + button (PATCH sets `companyId` + `status='converted'`), Drop button (DELETE → soft-delete), DetailTabs (Overview / Notes / Messages to Send / Replies / History); Overview is read-only until Edit and shows a formatted mailing **Address** block (via `lib/crm/address.ts` `formatAddress`) editable as six fields; the **History** tab has an "Add comment" composer (POST `/api/leads/[id]/comments`) and a unified timeline of comments + drafted/sent emails (merged via `lib/crm/timeline.ts` `buildLeadTimeline`); shows the business-card photo via `next/image` and the raw OCR text under Notes when present |
