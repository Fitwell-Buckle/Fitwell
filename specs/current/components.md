...
| `modules/production/supplier-leads/supplier-lead-form` | Client — supplier-pipeline twin of `leads/lead-form`. **Field order: Supplier type → Quick note → identity (email/name/phone/title/company/website) → Address.** No stage/source/persona. Plain email input (no Gmail typeahead). Accepts `initial`, `confidence`, `leadId` (edit → PATCH, else create → POST `/api/supplier-leads`), `submitLabel`, `onSuccess`, `secondaryLabel`/`onSecondary`, `rapid`. Confidence dots + collapsed Address in `rapid` mode, same as the lead form |
