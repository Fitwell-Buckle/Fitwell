...
| `return-drivers` | **Return Drivers** card (rendered below Customer Value & Retention) — small-multiples of the unit-level return rate (units returned ÷ units sold, from `order_refund_line` vs `order_line_item`) across nine dimensions: product family, size, color, products-in-order, time-to-refund, signal/came-from, time of day, day of week, order country. **Scoped to the selected date range + segment toggle** like every other card (`getReturnDrivers({from,to,segment})`); it does NOT apply the click-filters (it's the selector for `rd`, and a returns-only denominator would break the rate). Cells tinted vs the in-scope baseline (red ≥1.5×, amber ≥1.15×, green ≤0.6×); thin samples (<25 units) stay neutral. **Clicking any segment scopes the whole dashboard to that cohort** via the `rd` URL param (click again to clear), like the Returns Breakdown rows. Data from `lib/dashboard/return-drivers.ts`; the click→`rd`→WHERE-condition path is built from a single label source: `lib/dashboard/return-drivers-labels.ts` (labels + `parseRd`/`rdParam`) and `lib/dashboard/return-drivers-filter.ts` (`returnDriverFilter(dim,value)` → correlated SQL, threaded into every order query alongside `returnsCond`); pure helpers/types in `lib/dashboard/return-drivers-format.ts`. All three pure modules are unit-tested |
