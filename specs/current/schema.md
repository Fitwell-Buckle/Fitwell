...
| `creator_payout` | `creator_id` (FK, cascade, indexed), `period?` (free-form, e.g. "2026-07"), `amount_cents` (money in cents), `method?` (paypal\|wise\|other), `paid_at?`, `note?`, `created_by?` — ledger of commission **paid** (payout is manual v1); owed = earned − sum(paid), ~$25 floor. Migration 0097 |
