...
| `customer_message` | `gmail_message_id` (unique — dedup), `thread_id?`, `mailbox_user_id?` (FK → user) + `mailbox_label?` (whose inbox), `from_email`, `from_name?`, `subject?`, `snippet?`, `received_at?`, `audience` (`b2b`/`consumer`/`supplier`), `customer_id?` (FK → customer), `company_id?` (FK → company), `supplier_id?` (FK → supplier), `dismissed_at?`, `created_at`. Inbound emails from existing **customers or suppliers**, detected by the `customer-messages` cron (matches a sender's email to a stored `customer.email` / company contact / supplier contact email) across connected team inboxes. Surfaced at the top of the Customers B2B/Consumer tabs and the Suppliers list; `dismissed_at` hides one. Indexes: `audience`, `dismissed_at`, `received_at`, `customer_id`, `company_id`, `supplier_id` |
...
`customer` (consumer), company contact (b2b), or supplier contact (supplier) —
company wins, then supplier, then customer — recording new ones in
