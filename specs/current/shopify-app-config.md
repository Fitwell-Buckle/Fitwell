...
- **`write_discounts` + `read_shipping` grant (2026-07-01).** Both had sat declared-but-pending in the toml; released as `fitwell-admin-11` for the creator affiliate program (per-creator 15% discount codes) and creator market-gating. The "update permissions" banner did **not** surface (same custom-distribution quirk as `write_products`), so granted via uninstall → reinstall through the Partner Dashboard custom-distribution install link. Granted set confirmed by a `client_credentials` token exchange + `access_scopes.json`. **Lesson worth remembering: the reinstall wiped ALL 12 webhook subscriptions** — they're registered runtime-side (not in the toml), so order/customer/refund/product/collection events silently stopped firing until restored with `npm run shopify register-webhooks` (idempotent; re-points them at `https://portal.fitwellbuckle.co/api/webhooks/shopify`). Verify with `npm run shopify webhooks`. No prod redeploy was needed to flush the cached token — no deployed code used `write_discounts` yet, so the stale token (still holding all prior scopes) broke nothing; the next Phase-1 deploy refreshes warm-instance tokens for free.
...
| Scope released but no re-auth banner appears | Custom-distribution apps don't always surface a banner on scope change | Uninstall Fitwell Admin from `admin/settings/apps`, then reinstall via the Partner Dashboard's custom-distribution install link (Apps → Fitwell Admin → Distribution). Did this for `write_products` on 2026-06-08 and `write_discounts`+`read_shipping` on 2026-07-01 |
| Order / customer sync silently stops after an uninstall/reinstall | Reinstall drops **all** runtime-registered webhook subscriptions (they aren't declared in the toml) | Re-register: `npm run shopify register-webhooks` (idempotent, points at prod). Verify with `npm run shopify webhooks`. Always do this after any reinstall |
