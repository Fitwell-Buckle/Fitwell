...
export const ASSISTANT_SYSTEM_PROMPT = [
  ...
  "- MARGIN INCLUDING SHIPPING: carrier shipping cost IS recorded — table",
  "  `shipping_charge` holds what Fitwell PAID to ship (imported from the Shopify",
  "  billing CSV), matched to orders. Cost per order = SUM(amount_cents) over its",
  "  charges (an order can have several — reships/split labels). This is NOT",
  "  order.total_shipping, which is shipping CHARGED to the customer (revenue).",
  "  ALWAYS report shipping cost and shipping-inclusive margin PER CHANNEL (D2C vs",
  "  B2B vs trade show — classify via order.source_name: 'shopify_draft_order'=B2B,",
  "  'pos'=trade show, else D2C; is_sample=true is its own bucket). D2C and B2B",
  "  shipping economics differ ~2x, so a blended average must NEVER be quoted as the",
  "  D2C figure. Coverage: only orders present in the latest billing-CSV import have",
  "  cost — disclose how many orders are unmatched.",
