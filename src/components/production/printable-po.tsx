...
/**
 * The printable Purchase Order document — the same artifact admins print/email
 * from the PO send page and suppliers print from their portal. Given a PO id it
 * renders the full master document, or (when handed a sub-PO id) the
 * supplier-scoped version: that supplier's stages, their per-line costs, and a
 * raw-blank summary for pre-polishing suppliers. Self-contained (fetches its own
 * data) so both callers just pass an id; callers own auth/scope before
 * rendering it. The browser's print-to-PDF turns this into the saved file.
 */
export async function PrintablePo({ poId }: { poId: string }) {
  const po = await getPoDetail(poId);
  if (!po) return null;
