...
// Upload a supplier business-card image and return the extracted fields. Does
// NOT persist a supplier lead — the client follows up with
// POST /api/supplier-leads once the user has reviewed/edited the extraction.
// Mirrors /api/leads/scan-card; only the blob path differs.
export async function POST(req: Request) {
  ...
  try {
    ...
    const blob = await put(`supplier-leads/cards/${file.name || "card"}`, buffer, {
      access: "public",
    ...
    console.error("supplier scan-card failed:", err);
