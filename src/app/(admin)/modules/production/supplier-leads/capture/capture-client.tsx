...
// Reuse the generic, presentational capture components from the customer-lead
// capture flow — they're not lead-specific.
import { CardCamera } from "@/app/(admin)/leads/capture/card-camera";
import { QrScannerView } from "@/app/(admin)/leads/capture/qr-scanner-view";
import { type SupplierLeadFormInitial } from "../supplier-lead-form";
import { SupplierCaptureConfirm } from "./supplier-capture-confirm";
...
export function CaptureClient() {
  ...
  const [initial, setInitial] = useState<SupplierLeadFormInitial | null>(null);
  ...
  async function uploadCardFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/supplier-leads/scan-card", {
        method: "POST",
        body: form,
      });
      ...
      setInitial({
        ...
        phone: d.phone,
        title: d.title,
        // Supplier identified by email domain (the durable key), falling back
        // to any company name the model read off the card.
