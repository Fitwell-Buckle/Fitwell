...
export default function SupplierLoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res?.error) {
        setError(
          "We couldn't send a sign-in link to that address. Make sure it's the email your Fitwell contact added, or get in touch with us.",
        );
      } else {
        setSent(true);
      }
        ...
        <Card className="mt-8 p-6">
          {sent ? (
            ...
            <form onSubmit={submit}>
              <h1 className="text-sm font-semibold text-zinc-900">Supplier sign in</h1>
              <p className="mt-1 text-xs text-zinc-500">
                Enter the email Fitwell added for you. We&apos;ll send a one-time sign-in link.
              </p>
              <Input
                type="email"
