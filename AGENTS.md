...
- **Vercel CLI**: uses a separate config dir (`~/.vercel-fitwell`) so it doesn't collide with other Vercel projects on your machine. All `vercel` commands in this repo must use `npm run vc` or `vercel --global-config ~/.vercel-fitwell`. First-time setup (`npm run vc login` etc.) is in `specs/ops/contributor-setup.md` §3.
  - **PATH gotcha (esp. for agents / non-interactive shells):** the `vc`, `db:migrate:prod`, and `db:pending:prod` scripts call the `vercel` binary **directly** (not via a relative path), so the global npm bin dir must be on `PATH`. On Oliver's machine the CLI lives at `~/.npm-global/bin/vercel`, which is **not** on the default non-interactive `PATH` — so a bare `npm run db:pending:prod` fails with `vercel: command not found`. Run these with the bin dir prepended: `PATH="$HOME/.npm-global/bin:$PATH" npm run db:pending:prod` (and `…:prod` / `npm run vc` likewise). If `vercel` is genuinely missing, install it (`npm i -g vercel`) — don't assume it's absent just because the bare command isn't found.
